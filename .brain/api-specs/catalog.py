...
API_ENDPOINTS: tuple[ApiEndpoint, ...] = (
    ...
    ApiEndpoint("GET", "/api/v1/admin/audit/health", "v1", "admin", "Audit log dead man's switch", True, False, "stable"),
    ApiEndpoint("POST", "/api/v1/admin/audit/check-now", "v1", "admin", "Trigger manual dead man's switch", True, False, "stable"),
    ApiEndpoint("GET", "/api/v1/admin/locks", "v1", "admin", "Lista redlocks ativos", True, False, "stable"),
    # --- Admin / Pool + Slow queries + Retencao (5) ---
    ApiEndpoint("GET", "/api/v1/admin/pool", "v1", "admin", "DB pool stats", True, False, "stable"),
    ApiEndpoint("GET", "/api/v1/admin/slow-queries", "v1", "admin", "Slow queries (>200ms)", True, False, "stable"),
    ApiEndpoint("POST", "/api/v1/admin/retencao/run", "v1", "admin", "Trigger retencao job", True, False, "stable"),
    ApiEndpoint("GET", "/api/v1/admin/n8n/validate-wfs", "v1", "admin", "Valida WFs N8N (B12)", True, False, "stable"),
    ApiEndpoint("GET", "/api/v1/admin/backup/status", "v1", "admin", "Status ultimo backup DB", True, False, "stable"),
