...
def normalize_snapshots(sheets: dict) -> list[dict]:
    ...
    payload = (
        sheets
        if isinstance(sheets.get(SHEET_ORDER), list)
        else sheets.get("sheets", {})
    )
    if not payload:
        ...
        anomalies.append(
            {
                ...
                "signals": [
                    {"type": stype, "detail": detail} for stype, detail, _ in signals
                ],
                "orders": row["orders"],
                "revenue": row["revenue"],
                "returns": row["returns"],
...
def build_recommended_actions(
    ...
    top_reason, top_count = max(
        reason_totals.items(), key=lambda kv: kv[1], default=("", 0)
    )
    if top_count >= 25:
        actions.append(
            {
    ...
    if last_row.get("inventory_sellable"):
        unsellable_share = last_row.get("inventory_unsellable", 0) / max(
            last_row.get("inventory_sellable", 0)
            + last_row.get("inventory_unsellable", 1),
            1,
        )
...
def make_dashboard_payload(sheets: dict) -> dict:
    ...
    unsellable_share = (
        (inventory_unsellable / inventory_total) if inventory_total else 0.0
    )
