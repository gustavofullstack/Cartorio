...
def verify_v3_completion(
    repo_root: Path, evidence_root: Path, docs_root: Path
) -> tuple[bool, list[str]]:
    violations: list[str] = []
    ...
    if not hg_file.exists():
        ...
        for g_id, g_info in gates.items():
            if g_info.get("status") != "BLOCKED_HUMAN":
                violations.append(
                    f"Human Gate {g_id} is not BLOCKED_HUMAN (got {g_info.get(status)})"
                )

    # 3. Gold dataset check
    fixture_file = (
        repo_root / "backend" / "tests" / "fixtures" / "cartorio_eval_v3.jsonl"
    )
    if not fixture_file.exists():
        ...
        lines = [
            l
            for l in fixture_file.read_text(encoding="utf-8").splitlines()
            if l.strip()
        ]
        if len(lines) < 200:
            violations.append(
                f"Gold dataset must have at least 200 cases (got {len(lines)})"
            )
...
def main() -> int:
    ...
    is_pass, violations = verify_v3_completion(
        args.repo_root, args.evidence_root, args.docs_root
    )
    ...
    print(
        "[V3 COMPLETION GATE PASS] All objective completion gates passed successfully!"
    )
    return 0
