...
async def run_e2e(
    url: str, password: str | None, message: str, timeout: float
        ...
        async with websockets.connect(url, open_timeout=timeout) as ws:
            ...
            if challenge.get("event") != "connect.challenge":
                return (
                    ...
                    None,
                    f"challenge invalido: {challenge.get('event')}",
                )
            ...
            return (
                bool(content),
                latency,
                str(content) if content else json.dumps(response),
                None,
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "--message", default=DEFAULT_MESSAGE, help="mensagem para enviar"
    )
    parser.add_argument("--timeout", type=float, default=15.0)
    parser.add_argument(
        "--expect-timeout", type=float, help="fail se latency > N segundos (SLO check)"
    )
    ...
    ok, latency, response, error = asyncio.run(
        run_e2e(url, password, args.message, args.timeout)
    )
    ...
    if args.expect_timeout and latency > args.expect_timeout:
        print(
            f"[SLO BURN] Latency {latency:.3f}s > {args.expect_timeout}s (OpenClaw SLO 95% < 5s)"
        )
        return 3
    if latency > SLO_LATENCY_SECONDS:
        print(
            f"[SLO WARNING] Latency {latency:.3f}s > {SLO_LATENCY_SECONDS}s SLO target"
        )
