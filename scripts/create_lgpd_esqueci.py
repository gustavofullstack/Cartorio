{
    ...
    "nodes": [
                    ...
                    "assignments": [
                        {
                            "id": "a1",
                            "name": "cliente_id",
                            "value": "={{ $json.body.cliente_id || $json.body.query?.cliente_id || '' }}",
                            "type": "string",
                        },
                        {
                            "id": "a2",
                            "name": "request_id",
                            "value": "={{ $json.headers['x-correlation-id'] || $json.headers['X-Correlation-Id'] || $json.body.request_id || ('req-' + $now.toMillis()) }}",
                            "type": "string",
                        },
                    ]
                },
            ...
            "parameters": {
                ...
                "bodyParameters": {
                    "parameters": [
                        ...
                        {
                            "name": "metadata",
                            "value": "={{ JSON.stringify({workflow: '23-lgpd-esqueci', correlation: $('Extract Cliente ID').item.json.request_id}) }}",
                        },
                    ]
                },
                "options": {"timeout": 15000, "retry": {"maxTries": 3}},
        ...
        },
        {
            ...
            "parameters": {
                "respondWith": "json",
                "responseBody": "={{ JSON.stringify({status: 'ok', action: 'lgpd.esquecimento.executado', cliente_id: $('Extract Cliente ID').item.json.cliente_id, correlation: $('Extract Cliente ID').item.json.request_id, audit_id: $json.audit_id}) }}",
                "options": {},
            },
        },
        {
