...
def evaluate_reply(persona: dict[str, Any], reply: str, turn: int) -> dict[str, Any]:
    ...
    return {
        ...
        "words": words,
        "hard_ok": not any(
            i.startswith(("internal_leak", "wrong_institutional", "unrequested"))
            for i in issues
        ),
    }
...
def run_persona(slug: str, dry: bool = False) -> dict[str, Any]:
    ...
    name = persona["name"]
    print(
        f"=== PERSONA {name}, {persona['age']} anos ({persona['style']}) ===",
        flush=True,
    )
    ...
    for turn, msg in enumerate(persona["messages"], 1):
        ...
        if not dry:
            ...
            if not ok:
                transcript.append(
                    {"turn": turn, "user": msg, "reply": None, "status": "SEND_FAIL"}
                )
                continue
            resp = wait_reply(ts)
        else:
        ...
        if resp is None:
            transcript.append(
                {"turn": turn, "user": msg, "reply": None, "status": "TIMEOUT"}
            )
        ...
        transcript.append(
            {
                ...
                "n_assistant": resp["n_assistant"],
                "eval": ev,
                "status": "OK" if ev["hard_ok"] else "FAIL",
            }
        )
        flag = "✓" if ev["hard_ok"] else "✗"
        print(
            f"  [{turn}] {flag} warm={ev['warm_markers']} cold={ev['cold_markers']} "
            f"words={ev['words']} lat={resp['latency_s']}s {ev['issues']}",
            flush=True,
        )
        time.sleep(2)
    ...
    (ARTIFACTS / f"{slug}.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=1)
    )
    print(
        f"=== {name}: ok={n_ok} fail={n_fail} timeout={n_to} warm_avg={result['warm_avg']} ===",
        flush=True,
    )
    return result
