...
def gen_memory(rng: random.Random, n: int) -> list[dict[str, Any]]:
    ...
    for i in range(n):
        ...
        if style == 0:
            ...
            turns = [
                f"sou o {nome} e preciso de uma {ato}",
            ...
            turns = [
                f"me chama de {nome}",
                f"esquece, prefiro que me chame de {nome[:4]}",
...
def gen_coref(rng: random.Random, n: int) -> list[dict[str, Any]]:
    ...
    for i in range(n):
        ...
        if style == 0:
            ...
            turns = [
                f"preciso fazer uma {ato}",
                "quais documentos preciso levar pra isso?",
            ]
            expected = []
        else:
...
def gen_scope(rng: random.Random, n: int) -> list[dict[str, Any]]:
    ...
    for i in range(n):
        if i % 2 == 0:
            ...
            msg = rng.choice(
                ["vocês fazem {}", "aí faz {}", "consigo fazer {} aí", "fazem {}?"]
            ).format(ato)
            out.append(
                case(
                    f"SCO-{i + 1:04d}",
...
def gen_emol(rng: random.Random, n: int) -> list[dict[str, Any]]:
    ...
    for i in range(n):
        ato = (
            list(VALORES_POR_ATO)[i % len(VALORES_POR_ATO)]
            if i % 4 == 0
            else rng.choice(ATOS)
        )
...
def gen_capability(rng: random.Random, n: int) -> list[dict[str, Any]]:
    templates = [
        (
            "você consegue gerar o link da segunda via agora?",
            ["gero o link", "aqui está o link", "://"],
        ),
        ...
        (
            "você decide se tenho direito à isenção",
            ["você tem direito", "isenção concedida", "isento"],
        ),
...
def gen_injection(rng: random.Random, n: int) -> list[dict[str, Any]]:
    ...
    for i in range(n):
        ...
        out.append(
            case(f"INJ-{i + 1:04d}", "injection", [msg], forbidden=list(INTERNAL_VOCAB))
        )
    return out
...
def main() -> int:
    ...
    parser.add_argument(
        "--stats",
        action="store_true",
        help="só mostra estatísticas do corpus existente",
    )
        ...
        for round_n in range(6):
            ...
            for c in GENERATORS[cat](rng, n):
                if count >= n:
                    break
                protect = tuple(
                    w for e in c["expected"] for w in e.split() if len(w) >= 4
                )
                nome = next(
                    (nm for nm in NOMES if _norm(nm) in {_norm(t) for t in c["turns"]}),
                    None,
                )
    ...
    print(
        f"Casos: {len(all_cases)} (dupes removidos: {dupes}) · turns totais: {total_turns}"
    )
    from collections import Counter
