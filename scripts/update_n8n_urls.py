...
def main():
    ...
    if r.ok:
        ...
        for cred in creds:
            ...
            c_detail = (
                s.get(f"{BASE}/rest/credentials/{cred.get('id')}")
                .json()
                .get("data", {})
            )
            ...
            for k, v in data.items():
                if isinstance(v, str) and (
                    "localhost" in v or "http://api:" in v or "http://cartorio" in v
                ):
                    print(f"  Found stale URL in '{k}': {v}")
                    data[k] = (
                        v.replace("http://api:8000", "https://api.2notasudi.com.br")
                        .replace(
            ...
            if needs_update:
                ...
                upd = s.patch(
                    f"{BASE}/rest/credentials/{cred.get('id')}", json={"data": data}
                )
    ...
    if r.ok:
        ...
        for wf in wfs:
            ...
            for node in nodes:
                ...
                for k, v in params.items():
                    if isinstance(v, str) and (
                        "localhost" in v
                        or "http://api:" in v
                        or "http://cartorio" in v
                        or "http://host.docker.internal" in v
                    ):
                        print(
                            f"  Found stale URL in workflow {wf.get('name')} node {node.get('name')} param {k}: {v}"
                        )
                        params[k] = (
                            v.replace("http://api:8000", "https://api.2notasudi.com.br")
                            .replace(
            ...
            if needs_update:
                ...
                upd = s.patch(
                    f"{BASE}/rest/workflows/{wf.get('id')}", json={"nodes": nodes}
                )
                if upd.status_code == 405:
                    print(
                        "  PATCH not allowed, you must use POST or DB update (N8N 2.x)"
                    )
                else:
                    print(f"  Update result: {upd.status_code}")
    else:
