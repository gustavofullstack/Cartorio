...
| 2 | `01-consulta-emolumento.json` | 01 - Consulta Emolumento WhatsApp (v3) | ✅ | 11 | webhook, respondToWebhook | `Init Correlation`, `Webhook consulta-emolumento`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `PII Scrub` |
| 3 | `02-criar-protocolo.json` | 02 - Criar Protocolo (LGPD) | ✅ | 11 | webhook, respondToWebhook, respondToWebhook | `Init Correlation`, `Webhook WhatsApp`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `PII Scrub + LGPD Check` |
...
| 5 | `04-boas-vindas-lgpd.json` | 04 - Boas-Vindas + Consentimento LGPD | ✅ | 10 | webhook, respondToWebhook, respondToWebhook | `Init Correlation`, `Webhook Boas-Vindas`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Detectar Usuario` |
| 6 | `04-consulta-protocolo.json` | 04 - Consulta Protocolo | ✅ | 11 | webhook, respondToWebhook, respondToWebhook | `Init Correlation`, `Webhook WhatsApp`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Extrair Protocolo + PII` |
| 7 | `05-agendamento.json` | 05 - Agendamento Atendimento | ✅ | 11 | webhook, respondToWebhook, respondToWebhook | `Init Correlation`, `Webhook WhatsApp`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Parse dia/hora + PII` |
| 8 | `06-2-via-protocolo.json` | 06 - Segunda Via Documento | ✅ | 11 | webhook, respondToWebhook, respondToWebhook | `Init Correlation`, `Webhook WhatsApp`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Extrai protocolo + PII` |
...
| 11 | `10-faq-bot.json` | 10 - FAQ Bot | ✅ | 8 | webhook, respondToWebhook | `Init Correlation`, `Webhook WhatsApp`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `FAQ Knowledge Base` |
| 12 | `11-monitor-cartorio.json` | 11 - Monitor Cartório | ✅ | 18 | webhook, respondToWebhook, scheduleTrigger | `Init Correlation`, `POST /monitor-cartorio`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Check API` |
| 13 | `12-chatbot-llm-end-to-end.json` | 12 - Chatbot LLM End-to-End (PII + MCP + OpenCode-Go) | ❌ | 8 | webhook, respondToWebhook | `Webhook Evolution`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Extract Message Fields`, `PII Scrubber` |
| 14 | `14-opencode-go-fallback.json` | 14 - OpenCode-Go LLM Fallback (direct OpenAI-compat) | ❌ | 7 | webhook, respondToWebhook | `OpenClaw Fallback Trigger`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Extract Prompt`, `POST OpenCode-Go Direct` |
| 15 | `16-prospeccao-enrichment.json` | 16 - Prospeccao Lead Enrichment (Tier A/B/C scoring) | ✅ | 10 | webhook, respondToWebhook | `Init Correlation`, `Lead Novo Webhook`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Extract Lead` |
...
| 21 | `23-lgpd-esqueci-v2.json` | 23 - LGPD Esqueci (DELETE cliente + cascade + audit) | ❌ | 24 | webhook, respondToWebhook, respondToWebhook | `LGPD Esqueci Webhook`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Extract Cliente ID`, `Dedup Webhook (SETNX)` |
...
| 26 | `26-alerta-critico.json` | 26 - Alerta Critico (Telegram IM + Chatwoot) | ✅ | 10 | webhook, respondToWebhook | `Init Correlation`, `Alerta Webhook`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Extract Alerta` |
| 27 | `27-welcome-first-time.json` | 27 - Welcome First Time (consentimento LGPD) | ❌ | 9 | webhook | `Init Correlation`, `Welcome First Time`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Extract Cliente` |
...
| 31 | `31-telegram-listener.json` | 31 - Telegram Listener (CartorioBot test) | ✅ | 12 | webhook, respondToWebhook | `Init Correlation`, `Webhook Telegram`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Extract Telegram fields` |
| 32 | `33-whatsapp-qr-scan-helper.json` | 33-whatsapp-qr-scan-helper | ✅ | 6 | webhook | `Webhook Evolution Connection Trigger`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Is Connection Closed?`, `Notify Closed to Admin (Telegram)` |
...
| 34 | `35-llm-fallback-3x.json` | 35-llm-fallback-3x | ✅ | 8 | webhook | `Webhook Trigger`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Try Primary (opencode_go)`, `Try Secondary (openclaw)` |
| 35 | `36-chatwoot-telegram-sync.json` | 36-chatwoot-telegram-sync | ✅ | 9 | webhook | `Chatwoot Webhook Trigger`, `Dedup Webhook (SETNX)`, `Redis SETNX 24h`, `Filter: message_created only`, `Filter: outgoing messages only` |
...
- **googleCalendarTrigger**: 1 workflow(s)
