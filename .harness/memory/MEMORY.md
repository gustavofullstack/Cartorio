...
- **Lesson 278 — AGENT PIETRA: renomear Hermes + endurecer prompt + OCR de PDFs escaneados (2026-07-27)**: (1) Agente renomeado Hermes → Pietra em `~/.hermes/profiles/cartorio/SOUL.md`. Identidade canônica: **AGENT PIETRA · MINIMAX M3 1M XMAX**. Regras endurecidas no prompt: ZERO emoji, saudação contextual BRT (Bom dia 05-12h, Boa tarde 12-18h, Boa noite 18-05h), proibição absoluta de vazar info dev ("Testes confirmados", "o canal tá respondendo bem", menção a Kimi/GPT/Claude/Codex, emojis), anti-injeção reforçada, mensagens curtas quebradas por `---` quando o cliente pede lista. (2) Gateway Hermes reiniciado (PID 65548). Validação real iMessage: `hermes --profile cartorio -z` retornou (a) "Bom dia! 2º Tabelionato funciona de segunda a sexta das 9h às 17h…" sem emoji, (b) "Sou a Pietra, a agente do 2º Cartório de Notas de Uberlândia" (anti-leak OK), (c) "Procuração genérica (1 outorgante): R$ 68,94 + TFJ R$ 16,51" via tool MCP `cartorio_calcular_emolumento` (chamada real, não alucinação). (3) **Lesson crítica**: PDFs oficiais da RECOMPE/ÁRECOMPE-MINAS GERAIS são **escaneados** — `pdfplumber.extract_text()` retorna 0 chars, `pdftotext` retorna 14 bytes. Pipeline: `pdftoppm -r 300 -png` → `tesseract -l por`. OCR da Tabela Fixação 1 (14p, 28KB) + Fixação 8 (4p, 5.6KB) versionado em `docs/tjmg-ocr/`. Loader `app/services/tjmg_ocr_loader.py` com SHA-256 dos PDFs originais (marcado `# noqa: ALLOW_KEY_FALLBACK` — fingerprints públicos, não chaves). **Gotcha OCR**: separadores decimais (vírgula) frequentemente perdidos — `R$ 68,94` pode virar `R$ 68.94` ou `R$ 16618` (sem vírgula). Por isso a Pietra **NUNCA** cita valores de memória — sempre chama tool MCP. (4) **Anti-falso-positivo secret scanner**: SHA-256 hex64 de fingerprints públicos é confundido com `WEBHOOK_SECRET_HEX64` pelo `check_no_literal_keys.py`. Sempre anotar com `# noqa: ALLOW_KEY_FALLBACK` e motivo explícito.
- **Lesson 277 — Contrato público vs enriquecimento (CARTORIO_INFO, 2026-07-27)**: Campos canônicos protegidos por teste (`tests/test_cartorio_agent_public_profile.py`) — `endereco` e `horario` — NÃO devem ser sobrescritos por dados complementares (segunda unidade, horário estendido). Regra: se quiser adicionar informação, criar **chaves novas** (`unidade_secundaria`, `expedicao`, `titular`, `cnpj`, `instalacao`) sem mexer nas canônicas. DRY aplicado para `r.status_code in (403, 429)` repetido 3x entre providers → helper `_is_provider_rate_limited(status_code)` com docstring explicando porque 403 é alias de 429 (provider free exaure quota retornando 403 com payload de billing). 24/24 testes cartorio_agent + public_profile verdes.
