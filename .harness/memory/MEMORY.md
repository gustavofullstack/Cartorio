...
### Etapa 13 — Diagnóstico Operacional Hostinger VPS, Reconciliação do Health Radar e MiniMax M3 (2026-09-04)
- **Lesson 302 — Diagnóstico Hostinger VPS, Saneamento do Radar e FastMCP/MiniMax M3 (2026-09-04)**: VPS Hostinger (`187.77.236.77` / Tailscale `100.99.172.84`) auditada com uptime de 40+ dias. Evolution API (`cartorio_whatsapp-api`) online e conectada em estado OPEN (`cartorio-agent`). PostgreSQL 17 + pgvector na migration Alembic 0032. FastMCP autenticado com JWT Bearer token sob `/mcp/`. Sondas de health_radar e health_integracoes refatoradas para respeitar flags de servicos desativados (`openclaw_enabled=false`, `chatwoot_enabled=false`) e enviar headers de autorizacao na checagem de modelos MiniMax M3 (`/v1/models` HTTP 200). Identificado servico zumbi `cartorio_api` (0/1) no Swarm em crashloop continuo tentando puxar imagem inexistente enquanto `cartorio_system-api` e o container ativo.
...
- **Addendum WhatsApp (2026-08-18)**: o silêncio após o aviso inicial no canal vivo não era o `return` do SIM em si. O consentimento era resolvido só por `telefone_hash`; a pergunta seguinte frequentemente chega só em `@lid` sem `remoteJidAlt`. O gate via `has_consent=False` + debounce `consent:wa:notice:{lid}` devolvia `consent_required` sem reenviar LGPD. Fix: lookup por identidades de canal, bind LID+telefone no aceite, poll nativa Evolution `Sim`/`Nao` com fallback textual, voto estruturado `pollUpdateMessage` fora do failsafe de mídia. Testes: `tests/test_whatsapp_lgpd_poll_p0.py`. Deploy/smoke real continuam na VPS Agent Cartório via Tailscale — este ambiente não tem `tailscale`.
