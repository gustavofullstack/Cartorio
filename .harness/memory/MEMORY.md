...
## INDICE RAPIDO (atualizado 2026-07-24 ~Grok Wave Final P0 BRT)
...
- **Coverage 2026-07-24**: lote B `5755 passed / 1 failed / 21 skipped` + lote A crítico `192 passed`; `coverage report` consolidado **91%**. Falha residual: `test_dead_code_audit_g8::test_summary_has_clean_flags` (ruff_clean snapshot stale — não P0 runtime). Telegram-1000 = 1 teste async com **1000** webhooks concorrentes (`telegram1000` marker; desmarcado no addopts default).
