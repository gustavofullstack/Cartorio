...
### 2026-07-14 — Verificacao pos-R7 + Lesson 170/171/172/173
...
- **2026-07-14** Lesson 172 — R8 prod probe: **P0 CRÍTICO descoberto** — `https://api.2notasudi.com.br` retorna **HTTP 502 em TODOS os 14 endpoints testados** (incluindo `/health`, `/healthz`, `/mcp`, `/metrics`, `/api/v1/ws/atendimentos`); DNS+TLS+443 OK, mas TCP 8000 BLOCKED (`cartorio_api` sem réplicas healthy no Swarm). R3 status agora **indeterminado** (não "not in prod", mas "deploy seria invisível"). Checkpoint "Antes de inferir 'fix não deployed'": rodar `nc -zv 187.77.236.77 8000`. Se BLOCKED, é outage de camada — não inferir. Checklist completo de recovery (4 cenários: crash/tag errada/deadlock/rolling stuck) + deploy v0.6.0 + verificação R3 + rollback + env vars em `docs/R3_DEPLOY_STATUS.md`. Próxima ação é de Gustavo (SSH VPS), não auto-deploy. Full report in `lesson-172-r3-deploy-status-2026-07-14.md`
- **2026-07-14** Lesson 173 — Sprint 8 coverage push (renumbered from 171 by recovery c780aa4 → master, since slot 171 was already taken by PII-resolved lesson 4ab0bed): 94.09→95.04% (+0.95pp, gate --cov-fail-under=95 PASS) — 48 testes em `tests/test_sprint8_coverage.py` cobrindo bottom 5 modulos (main.py 83→92%, lgpd_direitos_v2.py 90→95%, notificacao.py 90→**100%**, integrations.py 95→99.5%, ws/atendimentos.py 87→96%, protocolo/backup_v2 → **100%**, deps.py 91→97.5%). Padroes reusaveis: (a) identificar bottom N por `coverage.json missing_lines` DESC NAO por %; (b) helpers privados primeiro (ROI alto, sem fixtures); (c) `_AsyncCtxExplode` para branches de exception em httpx; (d) HMAC n8n = `content=body` bytes (NAO `json=payload`); (e) lifespan loops exigem mock de `asyncio.sleep` da func + patches em `app.services.redlock` lazy-imported; (f) `pii.py` NAO modificado (LGPD sign-off NAO necessario p/ coverage tests em codigo de terceiros). Full report in `lesson-171-sprint8-coverage-95pct-2026-07-14.md` (filename kept — `lesson-171-` prefix is a pre-existing T1 label quirk; uniqueness preserved by suffix)
### Por tema (relevante)
