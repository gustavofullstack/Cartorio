...
- **Lesson 279 — AGENT PIETRA P0: Conversation State + Capability Registry + Cache snapshot bug (2026-07-27)**: **Causa raiz descoberta** via análise de `request_dump_*.json`: o gateway Hermes Agent tem um `.skills_prompt_snapshot.json` (79KB, congelado de 26/07 16:21) que re-emite o system prompt ANTIGO mesmo após `SOUL.md` ser reescrito + gateway reiniciado. Renomear SOUL.md não é suficiente — o snapshot precisa ser apagado. **Fix**: `rm .skills_prompt_snapshot.json` + `launchctl unload/load`. Validação: 4/4 testes reais `hermes -z "quem é você?"` retornaram "Sou a **Pietra**" (antes retornava "Sou o Hermes"). **(2) 3 módulos novos implementados** aplicando o PROMPT P0 (16 fases): `pietra_capabilities.py` (Capability Registry runtime-aware com 9 capabilities + gate can_say_i_can_do_it = registered AND tool_available AND runtime_healthy AND authorization_ok — reflete runtime real de 2026-07-27 com chatwoot/whatsapp_session/OpenClaw DOWN), `pietra_conversation_state.py` (L0-L4 layers + detect_scope_intent ALL/CONTINUE/SUMMARY_EACH + 60+ FORBIDDEN_PHRASES incluindo "minha memoria nao e grande" / "sou o hermes" / "testes confirmados" / "gero o link" / "transfiro agora" / emojis), `pietra_response_planner.py` (pipeline 14 steps: detect_intent → load_state → dedup → persona_filter → operational_truth_filter → update_state). **(3) 27 testes de regressão** (REG-001..REG-007 dos screenshots reais + 20 variantes) — todos PASS. **(4) 60/60 testes verdes** (27 novos + 33 existentes). Lint/mypy/secret-scan OK. **(5) Validação real iMessage** (5 testes pós-restart PID 70490): "Sou a Pietra" consolidado; sem vazamento de tools internas (memory/cron/skills/Agent Zero/MegaHub); emolumento da memória → "qual tipo?" (não cita valor). **Gotcha 2**: o gateway injeta automaticamente um system prompt adjacente listando tools internas; precisa regra explícita no SOUL.md: "Quando cliente pedir 'tudo que pode fazer', responda APENAS com o que o CARTÓRIO oferece. NUNCA liste ferramentas internas." **Gotcha 3**: cada `hermes -z` cria nova sessão (sem thread_id persistente) — conversation state só funciona em chamadas iMessage reais com thread fixo. Report: `docs/PIETRA_P0_HARDENING_REPORT.md`.
- **Lesson 278 — AGENT PIETRA: renomear Hermes + endurecer prompt + OCR de PDFs escaneados (2026-07-27)**:
