...
- **Lesson 276 — Stage 9 VAIO Purge + 14-Pilar VPS Diagnostic (2026-07-27)**: Remoção definitiva de TODAS as referências ao VAIO/agent-os/pc-linux-local do projeto (grep zero). Criado `docs/DIAGNOSTICO_VPS_MASTER_20260727.md` com diagnóstico real dos 14 pilares: 9 operacionais (FastAPI, Redis, Postgres, Telegram, Supabase, CNJ, Tailscale, MiniMax, Evo-Hub não necessário), 5 com bloqueios P0 (Hermes NOT_DEPLOYED — 4 Docker Secrets + stack deploy; WhatsApp sessão close — QR Code pendente; Chatwoot API 401 — credencial; iMessage NOT_DEPLOYED — depende Hermes; N8N chave parcial). Topologia definitiva: VPS Hostinger = TUDO, MacBook = SSH client. MiniMax key `sk-cp-*` em produção via Docker Secret `hermes_llm_api_key`, jamais em código fonte. `.brain/agents/README.md` reescrito com agents reais VPS. 7 arquivos de memória limpos.
