...
- [~] **E8.A16** Query slow log >200ms + endpoint /admin/slow-queries — owner: `cartorio-dev` — ⚠️ **PARCIAL**: `SlowLogMiddleware` (500ms threshold) já existe em `backend/app/middleware/slow_log.py` + registrado em `main.py:339`. FALTA: endpoint `/admin/slow-queries` + repositório persistente (Redis/DB) para consulta.
