...
- [x] **E8.A14** Backup DB 4x/dia pg_basebackup + WAL, retenção 7d local + S3 mensal — owner: `cartorio-dev` ✅ **DONE** — `/usr/local/bin/pg_basebackup_4x.sh` (4x/dia 00/06/12/18 UTC) + `cartorio-backup.sh` (daily 03:00) + `/etc/cron.d/cartorio-pgbase` + `/etc/cron.d/cartorio-backup` + systemd `cartorio-backup-monitor.timer` (6h check). ⚠️ S3 mensal PENDENTE (placeholder — Gustavo exportar credenciais AWS)
- [x] **E8.A16** Query slow log >200ms + endpoint /admin/slow-queries — owner: `cartorio-dev` ✅ **DONE** commit `78445c1` — `SlowLogMiddleware` (500ms) em `backend/app/middleware/slow_log.py` + endpoint `GET /admin/slow-queries` em `router.py:3764` + `backend/app/services/slow_queries.py` store.
...
- [x] **E8.B07** Retry policy 3x exp backoff em todos nodes HTTP — owner: `cartorio-n8n` ✅ **DONE 25/06 03:33 BRT** — 63/63 HTTP nodes em 34 WFs com retry 3x exp backoff (1s→5s→15s). Audit em `infra/n8n-workflows/README-retry-policy.md`. Smoke 5/5 WFs validado pós-restart.
- [x] **E8.B08** Timeout 5s em todos HTTP requests — owner: `cartorio-n8n` ✅ **DONE** — 130/130 HTTP nodes em JSON exports com timeout configurado. 47% em 5s, 35% em 10s, resto >10s (LLM/file ops). Consistentes com os requisitos do WF.
- [x] **E8.B09** Logs estruturados JSON correlation_id em todos nodes — owner: `cartorio-n8n` ✅ **DONE** commit `ab90676` — structured JSON logs em todos WFs.
