...
### N8N retry policy bulk apply — DB UPDATE canonico (B07 2026-06-25) (2026-06-25)
Type: lesson

**B07**: aplicar retry 3x exp backoff em 63 N8N HTTP nodes via DB UPDATE (NAO via API PATCH).

**Policy aplicada** (todos n8n-nodes-base.httpRequest sem retry):
```json
{"maxTries": 3, "waitBetween": [1000, 5000, 15000], "onWhenFailed": true}
```

**Por que DB UPDATE e nao API PATCH** (Lesson 96 confirmada):
- PATCH /api/v1/workflows/{id} → 405 Method Not Allowed
- PUT exige body completo (name, nodes, connections) — inviavel para update cirurgico
- DELETE + POST recreate perde versionId chain

**Padrao canonico 5 passos** (reforca Lesson 50+52+55):
1. Backup nodes/connections via \COPY base64 para /tmp/b07_wf_data.txt
2. UPDATE workflow_entity SET nodes, connections, versionId, activeVersionId, updatedAt
3. INSERT workflow_history FIRST (FK: workflow_entity.activeVersionId -> workflow_history.versionId)
4. docker service update --force cartorio_n8n + cartorio_n8n-runner
5. Validar via API GET em 5 sample WFs (100% cobertura esperada)
...
**Stats reais B07 03:33-03:42 BRT**:
...
- Smoke test: WF 23 LGPD Esqueci 4/4 validado via API GET
- 5 WFs re-validados pos-restart (TtD6qS6LCexwhMke, 4IS5oiLyHWGhtb8g, 5ABAZCQVRLd7AmM5, OYW3pxLCJFP47xgX, ITEGmC8k7nTJ78Uw) = 20/20 nodes
- Commit 9b2cc54 + Modified by Gustavo Almeida
- Total runtime: ~15min (vs Lesson 49 canonico ~3h30 sem dominar gotchas)

**Por que 100% em 15min**: aproveitei Lesson 49 (cache invalidation), Lesson 50 (UPDATE nodes+connections separados), Lesson 52 (restart), Lesson 55 (backend POST 404 lesson - nao apliquei, usei DB direto), Lesson 96 (PATCH 405 - nao tentei). Compor lições existentes > re-descobrir.
