...
### Lesson 187 — Sprint 3 parallel-session captured-my-work pattern (2026-06-29)
Type: lesson + trust-but-verify

**Cenario**: cartorio-dev session assigned 3 tasks (audit middleware / DELETE cliente / retenção scheduler). During my work session, Pietra root agent PARALLEL session committed `06b5c62 docs(specs)` + `5ec7b2c docs(validation): Turno 17` que INCUÍA meus 6 edits de audit_kwargs(request) em lgpd_direitos.py, meus settings retencao em config.py, meu scheduler loop em main.py, e meu test_audit_middleware_coverage.py — tudo sob commit messages de LGPD specs e Turno 17 docs.

**Achado**: quando fiz `git log --oneline -- backend/app/api/v1/lgpd_direitos.py` vi "06b5c62 docs(specs)" como último commit MAS o CONTEÚDO do arquivo era idêntico ao meu (6 audit_kwargs corretos, import de audit_context). Significa que outra sessão pegou meu WIP antes de eu commitar.

**Detecção canon (Lesson 5 reforça)**:
1. NÃO confie só em `git status` (working tree clean != nada pra fazer)
2. SEMPRE `git log --oneline -- <path>` em cada arquivo que VOCÊ editou pra ver último commit
3. Se for commit alheio MAS conteúdo idêntico ao seu = outra sessão capturou WIP
4. Solução pragmática: commits Task X com arquivos não-commitados pelos alheios + commit --allow-empty como marker formal da sua autoria

**Mitigação aplicada**:
- Task A: meus 4 arquivos já estavam em commits alheios. Usei `--allow-empty` com mensagem formal "feat(audit): middleware coverage 100% mutation log context" (db3242a).
- Task B (51613d0): test_delete_cliente_lgpd.py era UNTRACKED → commit normal com 375 insertions.
- Task C (cb4a3fa): retencao_scheduler.py + test_retention.py eram UNTRACKED → commit normal com 606 insertions.

**3 commits pushed**: cf455e0..db3242a (origin master aceitou).

**Pattern canon — sprint-mode-commit-races**:
- Sprint activo com 1+ agent paralelo = COMMITS SERÃO RACE
- Working tree pode ficar CLEAN antes do seu commit se outra sessão capturou
- Lesson 4/5/6 (briefing stale) + agora Lesson 187 (commit races): SEMPRE `git log -- file` antes de planejar qualquer coisa em sprint activo

**Lesson canon**: sprint-activo + agent-paralelo + git-status-clean NÃO significa "nada pra fazer". Significa "verifique se seu trabalho já foi commitado antes de fazer commit duplicado". Otimize pra evitar work duplication SEM dishonesty (nunca amend commit alheio; se conteúdo já em HEAD, marker --allow-empty com mensagem formal).
