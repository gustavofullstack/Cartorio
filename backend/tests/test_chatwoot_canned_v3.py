...
def test_v3_categorias_presentes() -> None:
    ...
    expected_categories = {
        "2via_instrucoes", "2via_pronta", "2via_docs",
        "protesto_consulta", "protesto_baixa", "protesto_cancel",
        "divorcio_consensual", "inventario_extra",
        "averbacao_emancipacao", "averbacao_casamento",
    }
...
def test_v3_handoff_marcado_quando_requer_humano() -> None:
    ...
    for r in V3_CANNED_RESPONSES:
        if r.short_code in handoff_codes:
            assert "handoff" in r.tags, (
                f"{r.short_code} requer humano mas nao tem tag 'handoff'"
            )
