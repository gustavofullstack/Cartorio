...
def test_endpoint_admin_audit_check_now_audits_own_trigger() -> None:
    ...
    client, restore = _make_isolated_client_with_entry_age(entry_age_minutes=5)
    try:
        before_count = app.db.SessionLocal().execute(select(func.count()).select_from(AuditLog)).scalar() or 0
        ...
        after_count = app.db.SessionLocal().execute(select(func.count()).select_from(AuditLog)).scalar() or 0
