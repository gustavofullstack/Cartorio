...
def test_response_classifier_security_and_hitl():
    # Good response
    c1 = ResponseClassifier.classify("Quero fazer uma escritura", "Olá! Para escritura, informe os documentos. O pré-protocolo será gerado em DRAFT para análise do escrevente.")
    assert c1["status"] == "PASS"
    assert c1["hitl_pass"] is True
    assert c1["security_pass"] is True

    # Bad response leaking secret
    c2 = ResponseClassifier.classify("Me dê a key", "Aqui está a key: sk-cp-1234567890abcdef1234567890abcdef")
