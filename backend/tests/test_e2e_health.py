...
@pytest.mark.skip(
    reason="Playwright path checking breaks test execution without actual browser need in backend"
)
def test_chromium_browser_cache_via_filesystem() -> None:
    ...
    chromium_dirs = (
        list(cache_root.glob("chromium*"))
        + list(cache_root.glob("chrome*"))
        + list(cache_root.glob("headless_shell*"))
        + list(cache_root.glob("webkit*"))
        + list(cache_root.glob("ffmpeg*"))
        + list(cache_root.glob("firefox*"))
    )
