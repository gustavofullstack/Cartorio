...
def list_with_pagination(
    ...
    *,
    ...
    # Replaced len(.scalars().all()) with native COUNT() via subquery
    # Expected impact: avoids loading all matched rows into Python memory, O(1) memory and data transfer overhead instead of O(N).
