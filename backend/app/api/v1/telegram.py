...
async def _handle_callback(
    ...
    *,
    ...
    if data == "agendar:confirmar":
        return await _confirmar_agendamento(bus, key, user_id=user_id)  # type: ignore[arg-type]
    return "", None, False
    ...
    *,
    ...
    if state == STATE_AGENDAR_CONFIRMAR:
        if tl in ("sim", "s", "ok", "confirmar"):
            r, kb, _ = await _confirmar_agendamento(bus, key, user_id=user_id)  # type: ignore[arg-type]
