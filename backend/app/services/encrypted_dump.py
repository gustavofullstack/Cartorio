...
def _key_material() -> bytes:
    raw = os.environ.get('BACKUP_FERNET_KEY') or os.environ.get('AUDIT_HMAC_KEY') or 'dev-backup-pepper-not-prod'
...
def verify_restore_route(
    *,
    ...
    checks.append(RestoreRouteCheck('decrypt_verified', decrypt_ok, 'decrypt roundtrip' if decrypt_ok else 'not verified'))
    checks.append(
        RestoreRouteCheck(
...
def build_encrypted_backup_manifest(result: EncryptedDumpResult, source: str = 'postgres') -> dict[str, Any]:
    return {
        'source': source,
        'algo': result.algo,
