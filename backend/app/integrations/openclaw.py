"""Integracao com OpenClaw (LLM provider low-cost primario).

OpenClaw expoe API compativel com OpenAI Chat Completions em
`{base_url}/chat/completions`.

Modelo roteado (LGPD RIPD v1.2 Tratamento 7):
    OpenClaw provider -> `deepseek-v4-flash` (compat OpenAI, low-cost)

IMPORTANTE — inconsistencia de modelo resolvida (auditoria cartorio-lgpd 2026-06-23):
- `.harness/reins/*/openclaw/openclaw.json` -> MiniMax-M2.7/M3 = config do **Mavis runtime**
...
- `backend/app/integrations/openclaw.py` -> `deepseek-v4-flash` = CORRETO para dados de cliente.
...
5. Fallback LiteLLM (TODO/placeholder) — se OpenClaw falhar, fallback com mesmo scrubbing
6. Docstring alinhada — modelo roteado eh `deepseek-v4-flash` via OpenClaw provider

Uso:
    from app.integrations.openclaw import chat, ChatError, ChatResponse
...
class ChatErrorKind:
    ...
    HTTP_5XX = "HTTP_5XX"  # Erro do servidor OpenClaw
...
def _check_rate_limit(
    ...
    """
    key = f"openclaw:ratelimit:{session_id}"
...
async def chat(
    ...
    *,
    ...
    """Chama OpenClaw Chat Completions com LGPD compliance.
    ...
    Args:
        ...
        api_key: API key do OpenClaw. Vazio => ChatError CONFIG.
        base_url: URL base do OpenClaw (sem /chat/completions).
    ...
    if not api_key or api_key.strip() == "":
        raise ChatError(
            "API key do OpenClaw nao configurada. Defina OPENCLAW_API_KEY no .env da VPS.",
            kind=ChatErrorKind.CONFIG,
        )

    if not base_url or base_url.strip() == "":
        raise ChatError(
            "Base URL do OpenClaw nao configurada. Defina OPENCLAW_BASE_URL no .env da VPS.",
            kind=ChatErrorKind.CONFIG,
        )
...
async def chat_with_settings(
    ...
    *,
    ...
    Raises:
        ChatError: CONFIG se OPENCLAW_API_KEY nao setado.
    ...
    return await chat(
        messages=messages,
        model=model or settings.openclaw_model,
